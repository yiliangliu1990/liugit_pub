...
This code is used to provide the secrecy capacity of Theorem 4 in [1].
...
Please change Nr and Nt to get corresponding results in Figs. 4, 5, 6, 7, 9, 10.
...
s = 1; % s < n
sc = zeros(nsample,1);
secrecy_capacity_monte = zeros(20,1);
secrecy_capacity_numerical = zeros(20,1);
eigen_capacity = zeros(20,1);
...
for p = 1:1:20
    ...
    for k = 1:1:s
        eigen_capacity(p) = eigen_capacity(p) + myeigencapacity(Nr,Nt,k,P/Nt);
    end
    secrecy_capacity_numerical(p) = eigen_capacity(p)+mycapacity(Ne,Nt-s,P/Nt)-mycapacity(Ne,Nt,P/Nt);
end
x = 1:1:20;
